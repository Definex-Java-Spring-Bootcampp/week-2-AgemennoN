...
public class UserService implements IUserService {
    ...
    public List<Application> getApplicationsByEmail(String email) {
        ...
        Optional<User> foundUser = userRepository.findByEmail(email);
        if (foundUser.isPresent()){
            return foundUser.get().getApplicationList();
        }
        return null;
    }

    @Override
    public Application createApplication(String email, Product product) {
        Optional<User> foundUser = userRepository.findByEmail(email);
        Application application = new Application(product, email, LocalDateTime.now());
        foundUser.get().getApplicationList().add(application);
        return application;
    }
}
